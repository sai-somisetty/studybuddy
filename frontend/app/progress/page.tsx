...
const stats = [
  { val:"68%", label:"Overall",    color:"#0E6655" },
  { val:"7",   label:"Day streak", color:"#E67E22" },
  { val:"142", label:"Questions",  color:"#1A1208" },
  { val:"78%", label:"Accuracy",   color:"#D4A017" },
];
const subjects = [
  { title:"Accounting", progress:72, color:"#0E6655" },
  { title:"Law",        progress:45, color:"#D4A017" },
  { title:"Maths",      progress:28, color:"#E67E22" },
];
  ...
  return (
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"20px 20px 100px", display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {stats.map((s,i) => (
            <motion.div key={i} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*0.06 }}
              ...
              <div style={{ fontSize:10, color:"#A89880", marginTop:2 }}>{s.label}</div>
            </motion.div>
          ))}
        </div>
        <div style={{ fontSize:12, fontWeight:600, color:"#6B6560" }}>Subjects</div>
        {subjects.map((s,i) => (
          <motion.div key={i} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1+i*0.06 }}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
              <span style={{ fontSize:13, fontWeight:500, color:"#1A1208" }}>{s.title}</span>
        ...
        <div style={{ fontSize:12, fontWeight:600, color:"#6B6560" }}>Needs attention</div>
        <div style={{ background:"#fff", borderRadius:16, padding:14, border:"0.5px solid rgba(220,38,38,0.2)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:13, fontWeight:600, color:"#1A1208", marginBottom:2 }}>Depreciation — AS 10</div>
              <div style={{ fontSize:10, color:"#A89880" }}>Quiz accuracy: 42%</div>
            </div>
            <motion.button whileTap={{ scale:0.97 }} onClick={() => router.push("/lesson")}
              style={{ background:"rgba(14,102,85,0.08)", color:"#0E6655", padding:"6px 12px", borderRadius:20, fontSize:11, fontWeight:600, border:"none", cursor:"pointer" }}>
              Study →
            </motion.button>
          </div>
        </div>
        <div style={{ background:"linear-gradient(135deg,#0A2E28,#0A4A3C)", borderRadius:16, padding:14, display:"flex", gap:10, alignItems:"flex-start" }}>
