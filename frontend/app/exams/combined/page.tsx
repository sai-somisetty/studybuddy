    ...
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"16px 20px 100px", display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontSize:12, fontWeight:600, color:"#4B6382" }}>Select chapters</div>
          <div style={{ fontSize:11, color: selected.length===3?"#071739":"#A4B5C4", fontWeight:600 }}>
          ...
          {Array.from({ length: chapters }, (_, i) => i+1).map(ch => {
            ...
            return (
              <motion.div key={ch} whileTap={!disabled ? { scale:0.97 } : {}}
                ...
                style={{ background: isSelected?"#071739": disabled?"#FAFAF8":"#fff", borderRadius:14, padding:14, border: isSelected?"2px solid #071739":"0.5px solid rgba(0,0,0,0.06)", cursor: disabled?"not-allowed":"pointer", textAlign:"center", opacity: disabled?0.5:1 }}>
                <div style={{ fontSize:13, fontWeight:600, color: isSelected?"#fff":disabled?"#A4B5C4":"#071739" }}>
                  Chapter {ch}
                </div>
