  ...
  return (
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"20px 20px 100px", display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ fontSize:12, fontWeight:600, color:"#4B6382" }}>Select subject</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {subjects.map(s => (
            <motion.div key={s.id} whileTap={{ scale:0.97 }}
              onClick={() => { setSubject(s.id); setChapter(0); }}
              style={{ background: subject===s.id?"#071739":"#fff", borderRadius:14, padding:14, border: subject===s.id?"2px solid #071739":"0.5px solid rgba(0,0,0,0.06)", cursor:"pointer", textAlign:"center" }}>
              <div style={{ fontSize:13, fontWeight:600, color: subject===s.id?"#fff":"#071739" }}>{s.label}</div>
              <div style={{ fontSize:10, color: subject===s.id?"rgba(255,255,255,0.6)":"#A4B5C4", marginTop:2 }}>{s.chapters} chapters</div>
            </motion.div>
          ))}
        </div>
        ...
        {selected && (
          <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}>
            <div style={{ fontSize:12, fontWeight:600, color:"#4B6382", marginBottom:10 }}>Select chapter</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {Array.from({ length: selected.chapters }, (_, i) => i+1).map(ch => (
                <motion.div key={ch} whileTap={{ scale:0.98 }}
                  onClick={() => setChapter(ch)}
                  style={{ background: chapter===ch?"linear-gradient(135deg,rgba(7,23,57,0.05),#DCFCE7)":"#fff", borderRadius:14, padding:"12px 16px", border: chapter===ch?"1.5px solid rgba(14,102,85,0.3)":"0.5px solid rgba(0,0,0,0.06)", display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer" }}>
                  <div style={{ fontSize:13, fontWeight: chapter===ch?600:400, color: chapter===ch?"#071739":"#071739" }}>
                    Chapter {ch}
                  </div>
                  {chapter===ch && <div style={{ fontSize:12, color:"#071739", fontWeight:700 }}>Selected ✓</div>}
                </motion.div>
              ))}
            </div>
          </motion.div>
