  ...
  return (
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"16px 20px 100px", display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#FEF9C3,#FFEDD5)", borderRadius:14, padding:12, border:"0.5px solid rgba(230,126,34,0.2)" }}>
          <div style={{ fontSize:12, fontWeight:700, color:"#9a3412", marginBottom:4 }}>⚠️ Use in final 4 weeks</div>
          <div style={{ fontSize:11, color:"#431407", lineHeight:1.5 }}>Full portion exams simulate real CA/CMA exam conditions. 60 questions, 90 minutes, full syllabus coverage.</div>
        </div>
        ...
        <div style={{ fontSize:12, fontWeight:600, color:"#6B6560" }}>Select paper (1–10)</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8 }}>
          {Array.from({length:10},(_,i)=>i+1).map(p => (
            <motion.div key={p} whileTap={{ scale:0.97 }}
              onClick={() => setPaper(p)}
              style={{ padding:"12px 0", borderRadius:12, background:paper===p?"#0A2E28":"#fff", border:paper===p?"none":"0.5px solid rgba(0,0,0,0.1)", cursor:"pointer", textAlign:"center" }}>
