...
function ResultsContent() {
  ...
  const passed  = pct >= 40;
  const color   = pct >= 80 ? "#16a34a" : pct >= 60 ? "#E67E22" : pct >= 40 ? "#185FA5" : "#DC2626";
  const bg      = pct >= 80 ? "linear-gradient(135deg,#F0FDF4,#DCFCE7)" : pct >= 40 ? "linear-gradient(135deg,#FFF7ED,#FFEDD5)" : "linear-gradient(135deg,#FEF2F2,#FEE2E2)";
    ...
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"20px 20px 100px", display:"flex", flexDirection:"column", gap:14, overflowY:"auto" }}>
        ...
        <motion.div initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
          ...
          <div style={{ fontSize:16, fontWeight:600, color:"#1A1208", marginBottom:4 }}>{score} of {total} correct</div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
          {[
            { val:score,       label:"Correct",  color:"#0E6655" },
            { val:total-score, label:"Wrong",    color:"#DC2626" },
            { val:total,       label:"Total",    color:"#1A1208" },
          ].map((s,i) => (
        ...
        <div style={{ background:"#fff", borderRadius:16, padding:14, border:"0.5px solid rgba(0,0,0,0.06)" }}>
          <div style={{ fontSize:12, fontWeight:700, color:"#1A1208", marginBottom:10 }}>By question type</div>
          {[
            { label:"Previous Papers",   color:"#E67E22" },
            { label:"Textbook Exact",    color:"#0E6655" },
            { label:"Tweaked",           color:"#185FA5" },
            { label:"AI Generated",      color:"#6B6560" },
          ].map((t) => (
            <div key={t.label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              ...
              <span style={{ fontSize:12, color:"#A89880" }}>Coming soon</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ textAlign:"center" }}>
          <span style={{ fontSize:11, color:"#A89880", cursor:"pointer" }}
