  ...
  return (
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"16px 20px 100px", display:"flex", flexDirection:"column", gap:12 }}>
        ...
        {/* Attempts list */}
        <div style={{ fontSize:12, fontWeight:600, color:"#4B6382" }}>Select attempt</div>
        {attempts.map((a, i) => (
          <motion.div key={a.name}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: selected===a.name?12:0 }}>
              <div>
                <div style={{ fontSize:14, fontWeight:600, color:"#071739" }}>{a.name}</div>
                <div style={{ fontSize:11, color:"#A4B5C4", marginTop:2 }}>{a.questions} questions · {subject}</div>
              </div>
              <div style={{ fontSize:12, color: selected===a.name?"#E3C39D":"#A4B5C4", fontWeight:600 }}>
            ...
            {selected===a.name && (
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
                ...
                <motion.button whileTap={{ scale:0.97 }}
                  ...
                  style={{ flex:1, padding:"10px", borderRadius:12, background: mode==="chapter"?"#071739":"rgba(7,23,57,0.05)", color: mode==="chapter"?"#fff":"#071739", fontSize:12, fontWeight:600, border:"none", cursor:"pointer" }}>
                  📖 Chapter Wise
                </motion.button>
                <motion.button whileTap={{ scale:0.97 }}
                  onClick={(e) => { e.stopPropagation(); setMode("full"); }}
                  style={{ flex:1, padding:"10px", borderRadius:12, background: mode==="full"?"#071739":"rgba(7,23,57,0.05)", color: mode==="full"?"#fff":"#071739", fontSize:12, fontWeight:600, border:"none", cursor:"pointer" }}>
                  ⏱️ Full Paper
                </motion.button>
              </motion.div>
            )}
