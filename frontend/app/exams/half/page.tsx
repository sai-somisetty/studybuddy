  ...
  return (
    <div className="app-shell">
      ...
      <div style={{ flex:1, padding:"16px 20px 100px", display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ fontSize:12, fontWeight:600, color:"#6B6560" }}>Which half?</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {[
            ...
            <motion.div key={h.id} whileTap={{ scale:0.97 }}
              ...
              style={{ background:half===h.id?"#0A2E28":"#fff", borderRadius:16, padding:16, border:half===h.id?"2px solid #0A2E28":"0.5px solid rgba(0,0,0,0.06)", cursor:"pointer", textAlign:"center" }}>
              <div style={{ fontSize:15, fontWeight:700, color:half===h.id?"#fff":"#1A1208", marginBottom:4 }}>{h.label}</div>
              <div style={{ fontSize:11, color:half===h.id?"rgba(255,255,255,0.7)":"#A89880", marginBottom:2 }}>{h.sub}</div>
              <div style={{ fontSize:11, color:half===h.id?"rgba(255,255,255,0.5)":"#6B6560" }}>{h.chapters}</div>
            </motion.div>
          ))}
        </div>
        ...
        {half && (
          <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}>
            <div style={{ fontSize:12, fontWeight:600, color:"#6B6560", marginBottom:10 }}>Select paper (1–5)</div>
            <div style={{ display:"flex", gap:8 }}>
              {[1,2,3,4,5].map(p => (
                <motion.div key={p} whileTap={{ scale:0.97 }}
                  onClick={() => setPaper(p)}
                  style={{ flex:1, padding:"12px 0", borderRadius:12, background:paper===p?"#0A2E28":"#fff", border:paper===p?"none":"0.5px solid rgba(0,0,0,0.1)", cursor:"pointer", textAlign:"center" }}>
                  <div style={{ fontSize:13, fontWeight:700, color:paper===p?"#fff":"#1A1208" }}>P{p}</div>
                </motion.div>
              ))}
            </div>
          </motion.div>
